...
flowchart TD
    ...
    LC1[Letter of Credit Issuance] --> BL_Check{Has original B/L arrived at buyer's bank?}
    ...
    Want_Goods -->|No| Wait[Wait for original B/L to arrive at the buyer's bank]

    Wait --> LC_Type

    SG1 --> SG_Flow[Buyer claims goods immediately using shipping guarantee]
    SG_Flow --> BL_Arrive[Wait for original B/L to arrive at the buyer's bank]
    BL_Arrive --> LC_Type
    ...
    Calc_Sight --> Need_Finance{Does the buyer need financing?}
    Calc_Usance --> Need_Finance

    Need_Finance -->|No| No_Finance_End[Wait for LC payment at maturity - no additional products needed]
    Need_Finance -->|Yes| BL_Name{Who is the original B/L issued under?}

    BL_Name -->|Buyer's name| PN1[P/N under Letter of Credit]
    BL_Name -->|Buyer's bank's name| TR1[Trust Receipt Loan under Letter of Credit]
    ...
    class LC1,SG1,PN1,TR1,Endorse1 productBox
```
